...
export default function DashboardClient({ membershipId, displayName, role, orgName, projects, activity, metrics, workload }: { membershipId: string; displayName: string; role: Role; orgName: string; projects: ProjectRow[]; activity: ActivityItem[]; metrics: Metrics; workload: WorkloadRow[] }) {
  ...
  return <main className={dark ? "app dark" : "app"}>
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><button className="nav-item" onClick={() => { if (role === "SUPER_ADMIN") router.push("/settings/branding"); else setActive("Settings"); }}><Icon name="settings"/><span>Settings</span></button><div className="profile"><div className="avatar dark-avatar">{initials(displayName)}</div><div><strong>{firstName}</strong><small>{roleLabel[role]}</small></div><span className="dots">•••</span></div></div>
    </aside>
